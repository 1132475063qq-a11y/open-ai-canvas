...
import { ArrowLeft, BookOpenText, Images, LayoutDashboard, LayoutGrid, PackageOpen, Plus, Settings2, type LucideIcon } from "lucide-react";
...
import EcommerceSettingsView from "./detail/ecommerce-settings";
import EcommerceWorkspaceView from "./detail/ecommerce";
import ProjectOverviewView from "./detail/overview";
import ProjectSettingsView from "./detail/settings";
import { isEcommerceProjectType, projectDetailDefaultView, projectDetailPath, projectDetailViewKeys, projectHubPath, type ProjectDetailViewKey } from "@/lib/project-domain";

type DetailView = ProjectDetailViewKey;

const shortDramaViews: Array<{ key: DetailView; label: string; shortLabel: string; icon: LucideIcon }> = [
...
const ecommerceViews: Array<{ key: DetailView; label: string; shortLabel: string; icon: LucideIcon }> = [
    { key: "ecommerce", label: "电商工作台", shortLabel: "工作台", icon: LayoutDashboard },
    { key: "ecommerce-assets", label: "商品资产", shortLabel: "资产", icon: PackageOpen },
    { key: "canvases", label: "项目画布", shortLabel: "画布", icon: LayoutGrid },
    { key: "settings", label: "项目设置", shortLabel: "设置", icon: Settings2 },
];
...
export default function ProjectDetailPage() {
    ...
    const isEcommerceProject = isEcommerceProjectType(detail.data?.project.type);
    const views = isEcommerceProject ? ecommerceViews : shortDramaViews;
    const defaultView = projectDetailDefaultView(detail.data?.project.type);
    const requestedView = view as DetailView | undefined;
    const activeView: DetailView = chapterId && !isEcommerceProject ? "chapters" : requestedView && projectDetailViewKeys(detail.data?.project.type).includes(requestedView) ? requestedView : defaultView;
    const createCanvas = () => {
    ...
    if (detail.isError || !detail.data) return <WorkspacePage><WorkspaceErrorState title="项目不可用" description="项目不存在、已被删除，或当前账号没有访问权限。" actionLabel="返回项目中心" onRetry={() => navigate(isEcommerceProject ? "/ecommerce" : "/projects")} /></WorkspacePage>;
    if (!isEcommerceProject && !shortDramaEnabled) return <WorkspacePage><WorkspaceState icon="empty" title="短剧创作暂未开放" description="当前功能已由平台管理员关闭。" action={<Button type="primary" onClick={() => navigate("/create", { replace: true })}>返回创作台</Button>} /></WorkspacePage>;
    if (isEcommerceProject && chapterId) return <Navigate to={`/projects/${projectId}/ecommerce`} replace />;
    if (!chapterId && (!view || !views.some((item) => item.key === view))) return <Navigate to={`/projects/${projectId}/${defaultView}`} replace />;
    const chapterHref = projectChapterHref(detail.data.units, projectId, chapterId);
    const projectHub = projectHubPath(detail.data.project.type);
    return (
                ...
                <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
                    <div className={activeView === "chapters" ? "min-h-0 flex-1" : "thin-scrollbar min-h-0 flex-1 overflow-y-auto px-3 py-5 sm:px-5 lg:px-8 lg:py-7"}>
                        <div className={activeView === "overview" ? "w-full" : activeView === "chapters" ? "h-full w-full" : "w-full"}>
                            {activeView === "ecommerce" ? <EcommerceWorkspaceView detail={detail.data} refreshProject={refreshProject} onCreateCanvas={createCanvas} /> : null}
                            {activeView === "ecommerce-assets" ? <EcommerceAssetsView detail={detail.data} refreshProject={refreshProject} onCreateCanvas={createCanvas} /> : null}
                            ...
                            {activeView === "settings" && isEcommerceProject ? <EcommerceSettingsView detail={detail.data} refreshProject={refreshProject} onCreateCanvas={createCanvas} /> : null}
                            {activeView === "settings" && !isEcommerceProject ? <ProjectSettingsView detail={detail.data} refreshProject={refreshProject} onCreateCanvas={createCanvas} /> : null}
                        </div>
                    </div>
                </main>
            </div>

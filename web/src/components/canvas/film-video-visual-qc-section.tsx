...
import { createFilmVideoVisualQCQuote, submitFilmVideoVisualQCQuote, type FilmProductionQCReport, type FilmVideoAttemptView, type FilmVideoVisualQCAttemptView, type FilmVideoVisualQCQuote } from "@/services/api/film-production";
import type { PublicLogicalModel } from "@/services/api/logical-models";
import { uploadResourceFile } from "@/services/api/resources";

type FilmVideoVisualQCSectionProps = {
    projectId: string;
...
type UploadedSample = { timeMs: number; resourceId: string };

type VideoQCDimension = {
    dimension: string;
    decision: string;
    issueCodes: string[];
    observations: string[];
...
const dimensionLabels: Record<string, string> = {
    ...
    lip_sync_audio: "口型与声音",
};

export function FilmVideoVisualQCSection({ projectId, attempt, models, onChanged }: FilmVideoVisualQCSectionProps) {
    const [modelId, setModelId] = useState("");
    const [quote, setQuote] = useState<FilmVideoVisualQCQuote | null>(null);
    ...
    const sourceAttemptId = attempt.attempt.id;
    const visualAttempts = attempt.visualQcAttempts || [];
    const latestVisualAttempt = visualAttempts.at(-1);
    const activeVisualAttempt = visualAttempts.find((item) => ["queued", "running", "uncertain"].includes(item.attempt.status));
    ...
    const quoteMutation = useMutation({
        mutationFn: async () => {
            let samples = uploadedSamples;
            if (!samples.length) {
                setPreparationStage("正在采样视频");
                const frames = await captureVideoQCFrames(attempt.result!.url!);
                setPreparationStage("正在上传采样证据");
                const resources = await Promise.all(
                    frames.map((frame) =>
                        uploadResourceFile(frame.blob, "image", {
                            width: frame.width,
                            height: frame.height,
                            fileName: `film-video-qc-${sourceAttemptId}-${frame.timeMs}.jpg`,
                        }),
                    ),
                );
                samples = frames.map((frame, index) => ({ timeMs: frame.timeMs, resourceId: resources[index].id }));
                setUploadedSamples(samples);
            }
            setPreparationStage("正在生成报价");
            return createFilmVideoVisualQCQuote(
                projectId,
                { sourceAttemptId, logicalModelId: modelId, sampleFrames: samples },
                operationKey(quoteKeyRef, "film-video-visual-qc-quote", JSON.stringify([sourceAttemptId, modelId, samples])),
            );
        },
        onSuccess: (nextQuote) => {
            setQuote(nextQuote);
    ...
    const submitMutation = useMutation({
        mutationFn: () =>
            submitFilmVideoVisualQCQuote(
                projectId,
                quote!.id,
                { quoteFingerprint: quote!.quoteFingerprint },
                operationKey(submitKeyRef, "film-video-visual-qc-submit", quote!.id),
            ),
        onSuccess: () => {
            setQuote(null);
            setConfirmSubmit(false);
    ...
    const canQuote = Boolean(attempt.attempt.status === "succeeded" && attempt.result?.url && modelId && !activeVisualAttempt);
    ...
    return (
        <section className="space-y-2 border-t pt-2" style={{ borderColor: "var(--border)" }}>
            ...
            {activeVisualAttempt ? <InlineState text={`${visualQCStatusLabel(activeVisualAttempt.attempt.status)} · ${activeVisualAttempt.task.stage}`} /> : null}
            {quote ? (
                <div className="space-y-2 rounded-lg p-2.5" style={{ background: "var(--library-surface)" }}>
                    <div className="flex items-center justify-between gap-2 text-xs">
                        <span className="truncate">{quote.model} · {quote.sampleFrames.length} 帧 · 十维检查</span>
            ...
            {latestVisualAttempt ? <VideoVisualQCAttemptState attempt={latestVisualAttempt} /> : <InlineState text="尚未运行视频模型视觉 QC" />}
            {error ? <Alert type="error" showIcon message={readError(error)} /> : null}
            <Modal title="确认视频视觉 QC 费用" open={confirmSubmit} onCancel={() => setConfirmSubmit(false)} onOk={() => submitMutation.mutate()} confirmLoading={submitMutation.isPending} okText="确认扣费并运行" cancelText="返回" centered>
                {quote ? (
                    <p className="text-sm">
                        将使用 {quote.model} 检查当前视频的 {quote.sampleFrames.length} 张采样证据，预计消耗 <strong>{quote.cost.required ? `${formatCredits(quote.cost.amountMicrocredits)} 积分` : "免费额度"}</strong>。模型结论不会自动接受视频。
                    </p>
                ) : null}
            </Modal>
        </section>
...
function VideoVisualQCAttemptState({ attempt }: { attempt: FilmVideoVisualQCAttemptView }) {
    const dimensions = useMemo(() => videoQCDimensions(attempt.report), [attempt.report]);
    return (
        <div className="space-y-2 rounded-lg p-2.5" style={{ background: "var(--library-surface)" }}>
            <div className="flex items-center justify-between gap-2 text-xs">
                <Tag color={attemptTone(attempt.attempt.status)}>{visualQCStatusLabel(attempt.attempt.status)}</Tag>
                <span className="min-w-0 truncate" style={{ color: "var(--foreground-muted)" }}>{attempt.sampleFrames.length} 帧 · {attempt.task.stage}</span>
            </div>
            {!attempt.valid ? <div className="text-xs" style={{ color: "var(--status-warning)" }}>视频版本已变化，此结论不再适用于当前槽位。</div> : null}
            ...
            {attempt.report ? (
                <>
                    <div className="flex items-center gap-1.5 text-xs" style={{ color: "var(--status-warning)" }}>
                        ...
                        <span>{modelVerdictLabel(attempt.report.decision)}，仍需人工裁决</span>
                    </div>
                    <details>
                        <summary className="cursor-pointer text-xs font-medium">查看十维采样证据</summary>
                        <div className="mt-2 divide-y" style={{ borderColor: "var(--border-semantic)" }}>
                            {dimensions.map((dimension) => (
                                <div key={dimension.dimension} className="space-y-1 py-2 text-xs">
                                    ...
                                    {dimension.observations.map((observation, index) => <div key={`${dimension.dimension}-${index}`} style={{ color: "var(--foreground-muted)" }}>{observation}</div>)}
...
function videoQCDimensions(report?: FilmProductionQCReport): VideoQCDimension[] {
...
function readError(error: unknown) {
    return error instanceof Error ? error.message : "视频视觉 QC 请求失败";
}

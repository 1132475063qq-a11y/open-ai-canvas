...
func TestOpenAIImageAdapterAcceptsTopLevelResultURLs(t *testing.T) {
	adapter, ok := Builtins().Get("openai-image")
	if !ok {
		t.Fatal("adapter missing")
	}
	result, err := adapter.ParseCreate(context.Background(), []byte(`{"result_urls":["https://cdn.example/first.png","https://cdn.example/second.png"]}`))
	if err != nil {
		t.Fatal(err)
	}
	if result.Status != StatusSucceeded || result.Result == nil || len(result.Result.Images) != 2 {
		t.Fatalf("result = %#v", result)
	}
	if result.Result.Images[0].URL != "https://cdn.example/first.png" || result.Result.Images[1].URL != "https://cdn.example/second.png" {
		t.Fatalf("images = %#v", result.Result.Images)
	}
}

...
func (w *taskLifecycleCoordinator) cancelTask(_ context.Context, userID string, id string) (*model.Task, error) {
	...
	if task.Status == model.TaskStatusQueued && model.IsFilmManagedTaskType(task.Type) {
		// Film 报价提交后先进入队列；在 worker 发起上游请求前取消仍然安全，
		// 需要与 Film Attempt 终态和预扣积分一起原子收尾。
		cancelled, err := s.repo.CancelTaskIfStatus(userID, task.ID, model.TaskStatusQueued, time.Now())
		if err != nil {
			return nil, err
		}
		if cancelled {
			if err := s.taskBilling().RefundBilling(task.BillingOrderID, "任务在调用上游前取消"); err != nil {
				return nil, err
			}
			task, err = s.repo.TaskForUser(userID, id)
			if err != nil {
			...
			_ = s.log(userID, task.ID, "warn", "Film 任务已取消并退还预扣积分", "")
			return taskForOutput(*task), nil
		}
		task, err = s.repo.TaskForUser(userID, id)
		if err != nil {
	...
	// 普通任务创建后可能已经产生上游费用。取消接口保留用于兼容旧客户端，

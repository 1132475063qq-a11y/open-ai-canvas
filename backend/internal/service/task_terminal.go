...
type taskTerminalCoordinator struct {
	...
	updateTerminal    func(*model.Task, model.TaskStatus, model.FilmProductionAttemptStatus) (bool, error)
}
...
func newTaskTerminalCoordinator(s *Service) *taskTerminalCoordinator {
	return &taskTerminalCoordinator{
		...
		updateTerminal:    s.updateTaskTerminalState,
	}
}
...
func (c *taskTerminalCoordinator) markPreparationFailure(task *model.Task, stage string, err error, billingUncertain bool, refundReason string) error {
	...
	if terminalErr := c.markTerminalState(task, filmStatus); terminalErr != nil {
...
func (c *taskTerminalCoordinator) handleExecutionFailure(task *model.Task, err error, providerSucceeded bool, channelSlotFailedBeforeRequest bool) error {
	...
	task.Stage = "任务失败"
	task.Error = c.userFacingMessage(err)
	requiresBillingReview := providerSucceeded || (!channelSlotFailedBeforeRequest && c.billing.BillingFailureRequiresReview(task.BillingOrderID, task.ID, err))
	filmStatus := model.FilmProductionAttemptStatusFailed
...
func (c *taskTerminalCoordinator) handleResultPersistenceFailure(task *model.Task, saveErr error) (handled bool, err error) {
	...
	if terminalErr := c.markTerminalState(task, model.FilmProductionAttemptStatusUncertain); terminalErr != nil {
...
func (c *taskTerminalCoordinator) markTerminalState(task *model.Task, filmStatus model.FilmProductionAttemptStatus) error {
	completedAt := time.Now()
	task.CompletedAt = &completedAt
	var err error
	if c.updateTerminal != nil {
		_, err = c.updateTerminal(task, model.TaskStatusRunning, filmStatus)
	} else {
		_, err = c.repo.UpdateTaskTerminalState(task.ID, model.TaskStatusRunning, task.Status, task.Stage, task.Error, completedAt)
	}
	if err != nil {

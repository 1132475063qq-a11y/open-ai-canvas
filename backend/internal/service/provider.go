	...
	for deadline := providerPollingDeadline(ctx); time.Now().Before(deadline); {
		...
		if status == "completed" || status == "succeeded" || status == "success" || status == "done" {
			if videoURL := newAPIVideoResultURL(state); videoURL != "" {
				...
				if err != nil && input.Config.InterfaceType == "xai-video" {
					// xAI may return a loopback or otherwise unreachable result URL;
					// the authenticated content endpoint is the authoritative fallback.
					data, mimeType, err = getBinary(ctx, input.Config, "/videos/"+id+"/content")
				}
				if err != nil {
